...
func TestLossGECommand_Run_NoContainers(t *testing.T) {
	...
	cmd, err := NewLossGECommand(mockClient, gparams, nparams, 0, 10.0, 20.0, 30.0, 40.0)
...
func TestLossGECommand_Run_DryRun(t *testing.T) {
	...
	expectedReq := &container.NetemRequest{
		Container: target,
		Interface: "eth0",
		Command:   []string{"loss", "gemodel", "10.00", "20.00", "30.00", "40.00"},
		Duration:  100 * time.Millisecond,
		Sidecar:   container.SidecarSpec{Image: "tc-image"},
		DryRun:    true,
	}
...
func TestLossGECommand_Run_WithRandom(t *testing.T) {
	mockClient := container.NewMockClient(t)
