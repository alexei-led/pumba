...
func Test_tcContainerCommands(t *testing.T) {
	...
	// pull image
	engineClient.On("ImagePull", ctx, config.Image, image.PullOptions{}).Return(io.NopCloser(readerResponse), nil)
	// create container
...
func Test_dockerClient_stressContainerCommand(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mockInit: func(ctx context.Context, engine *mocks.APIClient, conn *mockConn, targetID string, stressors []string, img string, pool bool) {
				// pull response
				pullResponse := imagePullResponse{
					Status:   "ok",
					Error:    "no error",
				...
				pullResponseByte, _ := json.Marshal(pullResponse)
				readerResponse := bytes.NewReader(pullResponseByte)
				engine.On("ImagePull", ctx, img, image.PullOptions{}).Return(io.NopCloser(readerResponse), nil)
		...
		},
		{
			...
			mockInit: func(ctx context.Context, engine *mocks.APIClient, conn *mockConn, targetID string, stressors []string, img string, pool bool) {
				engine.On("ImagePull", ctx, img, image.PullOptions{}).Return(io.NopCloser(bytes.NewReader([]byte("{}"))), errors.New("failed to pull image"))
			},
			wantErr: true,
		},
		{
