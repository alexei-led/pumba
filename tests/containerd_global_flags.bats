...
@test "Should target containers by regex pattern via containerd" {
    ...
    wait_for_running docker ctr_flag_victim_1
    wait_for_running docker ctr_flag_victim_2
...
@test "Should select random container with --random flag via containerd" {
    ...
    # Poll for exactly one to transition (--random is non-deterministic).
    wait_for 5 '
        docker inspect -f "{{.State.Status}}" ctr_flag_victim_1 2>/dev/null | grep -q exited ||
        docker inspect -f "{{.State.Status}}" ctr_flag_victim_2 2>/dev/null | grep -q exited' \
        "one victim to exit"
    local exited=0
