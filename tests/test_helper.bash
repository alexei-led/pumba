...
# Skip the current test unless podman is installed and its socket is reachable.
# Uses `podman info` as a liveness probe (covers both CLI availability and
# socket/daemon reachability in a single call).
require_podman() {
	if ! command -v podman >/dev/null 2>&1; then
		skip "podman CLI not available on PATH"
	fi
	if ! podman info >/dev/null 2>&1; then
		skip "podman daemon/socket not reachable"
	fi
}

# Get the PID of a Podman container for nsenter.
_podman_container_pid() {
	podman inspect -f '{{.State.Pid}}' "$1"
}

# Remove a podman container by name/id, tolerating missing containers.
podman_rm_force() {
	local name
	for name in "$@"; do
