...
@test "Should tag sidecar with pumba skip label to avoid recursive chaos" {
    # Start pumba in background; while running, the sidecar must carry the
    # com.gaiaadm.pumba.skip=true label so another invocation with re2: filter
    # cannot accidentally target it.
    pumba --runtime podman --log-level debug netem --interface dummy0 --tc-image docker.io/nicolaka/netshoot:latest --pull-image=false --duration 30s delay --time 50 pdm-sidecar-target &
    PUMBA_PID=$!

    wait_for 10 "[ -n \"\$(podman ps -q --filter 'label=com.gaiaadm.pumba.skip=true')\" ]" "sidecar to appear with skip label"

    run podman ps --filter "label=com.gaiaadm.pumba.skip=true" --format '{{.Names}}'
    echo "Sidecars: $output"
    [ -n "$output" ]
...
@test "Should clean up sidecar on SIGTERM to pumba via podman runtime" {
    ...
    # Confirm sidecar is up
    run podman ps -q --filter "label=com.gaiaadm.pumba.skip=true"
    [ -n "$output" ]

    # SIGTERM pumba; it must tear the sidecar down rather than leaking it
    ...
    # Target's netem rules must also be removed
    target_pid=$(podman inspect -f '{{.State.Pid}}' pdm-sidecar-target)
    run sudo nsenter -t "$target_pid" -n tc qdisc show dev dummy0
