...
2. Use the `--tc-image` or `--iptables-image` flags to specify a helper image (recommended). A custom netem helper image must include `sh`, `tail`, and `tc`.

### Recommended Images

| Image                                             | Base   | Includes                   |
| ------------------------------------------------- | ------ | -------------------------- |
| `ghcr.io/alexei-led/pumba-alpine-nettools:latest` | Alpine | sh + tail + tc + iptables |
| `ghcr.io/alexei-led/pumba-debian-nettools:latest` | Debian | sh + tail + tc + iptables |
