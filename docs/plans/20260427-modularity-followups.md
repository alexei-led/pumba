...
- [ ] write `pkg/chaos/netem/parse.go` with `ParseRequestBase(c cliflags.Flags, gp *chaos.GlobalParams) (*container.NetemRequest, error)` — reads parent flags via `c.Parent()`, fills `Container=nil` (set per-iteration), `Interface`, `IPs`, `SPorts`, `DPorts`, `Duration`, `Sidecar`, `DryRun`
- [ ] write `pkg/chaos/netem/parse_test.go` — table-driven cases for happy path, invalid duration, invalid CIDR, invalid port
- [ ] convert `pkg/chaos/netem/cmd/delay.go::parseDelayParams` to call `ParseRequestBase` + augment `req.Command` (pilot to confirm shape)
- [ ] convert remaining 6 netem cmd parsers (`loss`, `loss_ge`, `loss_state`, `rate`, `corrupt`, `duplicate`)
- [ ] convert each per-action chaos file (`pkg/chaos/netem/{delay,loss,...}.go`) to consume the request directly — drop the field-by-field copy at every callsite
- [ ] delete `Params` struct + `newNetemCommand` + `netemCommand` from `pkg/chaos/netem/netem.go`
- [ ] update `pkg/chaos/netem/*_test.go` — replace `Params{...}` literals with `*NetemRequest{...}`; mock expectations unchanged (already use `*container.NetemRequest`)
- [ ] update `pkg/chaos/netem/cmd/*_test.go` — assert on the request fields directly
...
- [ ] write `pkg/chaos/iptables/parse.go` with `ParseRequestBase(c cliflags.Flags, gp *chaos.GlobalParams) (*container.IPTablesRequest, error)` — reads `--protocol`, `--source`, `--destination`, `--src-port`, `--dst-port`, `--iptables-image`, `--pull-image`, `--duration` from parent flags
- [ ] write `pkg/chaos/iptables/parse_test.go` — table-driven cases mirroring netem parse_test
- [ ] convert `pkg/chaos/iptables/cmd/loss.go::parseLossParams` to build `*IPTablesRequest` directly (CmdPrefix/CmdSuffix per the action)
- [ ] convert `pkg/chaos/iptables/loss.go` to consume the request — `runIPTables` already takes `addReq, delReq *container.IPTablesRequest`
- [ ] delete `Params` struct + `newIPTablesCommand` + `ipTablesCommand` from `pkg/chaos/iptables/iptables.go`
- [ ] update `pkg/chaos/iptables/*_test.go` — replace `Params{...}` literals with `*IPTablesRequest{...}`
- [ ] update `pkg/chaos/iptables/cmd/*_test.go`
- [ ] `CGO_ENABLED=0 go test ./...` — all green
- [ ] `make lint` — must pass before next task
