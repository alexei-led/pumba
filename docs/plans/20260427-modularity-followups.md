...
- [x] update `startNetemContainer` to take `(ctx, *ctr.NetemRequest)` — read fields from request, no caller change outside the file
- [x] update `stopNetemContainer` similarly
- [x] update `startNetemContainerIPFilter` similarly
- [x] verify `tcCommands` helper signature is consistent — keep it taking primitive args since it's truly low-level (no leakage)
- [x] update `ipTablesContainer` + `ipTablesContainerWithIPFilter` similarly
- [x] verify no other callsite for these helpers (grep `git grep -n 'startNetemContainer\|ipTablesContainerWithIPFilter'`) — only in-file callers
- [x] update unit tests if any directly call these helpers — none do; all go through public methods
- [x] `CGO_ENABLED=0 go test ./...` — all green
- [x] `make lint` — 0 issues
...
- [x] create `pkg/runtime/containerd/netem.go` — moved `NetemContainer`, `StopNetemContainer`, `runTCCommands` (note: `sidecarExec` is shared with iptables, stays in `sidecar.go`)
- [x] create `pkg/runtime/containerd/iptables.go` — moved `IPTablesContainer`, `StopIPTablesContainer`, `runIPTablesCommands`
- [x] create `pkg/runtime/containerd/stress.go` — moved `StressContainer`, `stressDirectExec`, `stressSidecar` entry point; sidecar provisioning machinery (`createStressSidecar`, `buildStressSpecOpts`, `startSidecarTask`, `waitStressSidecar`, `deleteContainer`) lives in new `stress_sidecar.go` to keep both files under 250 LOC
- [x] create `pkg/runtime/containerd/cgroup.go` — moved `cgroupReader`, `isSystemdCgroup`, `cgroupChildPath`, `resolveCgroupPath`
- [x] trim `pkg/runtime/containerd/sidecar.go` to provisioning + reap helpers shared by netem/iptables (`sidecarExec`, `runSidecarCmd`, `pullImage`, `cleanupSidecar`, networkCapabilities + cleanup timeout consts) — 156 LOC
- [x] test code stays in `client_test.go` — fixtures and helpers are cross-concern; no production-LOC violation since `client_test.go` is a test file
- [x] verified each resulting production file is ≤ 250 LOC: client 243, stress_sidecar 206, sidecar 156, commands 117, cgroup 71, container 80, iptables 49, netem 45, stress 84, task 222, api 18
- [x] `CGO_ENABLED=0 go test ./...` — all green
- [x] `make lint` — 0 issues
...
- [x] `wc -l pkg/runtime/containerd/*.go` — every production file ≤ 250 LOC (largest: client.go 243, stress_sidecar.go 206, task.go 222)
...
- [x] `make test` — all 18 packages pass with race detector
- [x] `make test-coverage` — statement coverage 72.9% (≥ 71.9% baseline)
- [x] `make lint` — 0 issues from 65 active linters
- [x] `make build` — linux/amd64 + darwin/arm64 binaries build clean
- [x] manual smoke: `pumba --help`, `pumba stress --help`, `pumba rm --help`, `pumba netem delay --help`, `pumba iptables loss --help` — all show expected help, no flag/command regressions
- [x] run bats integration tests (Docker, containerd, Podman) — skipped (defer to release-time per Post-Completion; no daemons available in this session)
